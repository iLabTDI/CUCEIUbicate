...
const verifyPassword = (inputPassword, storedHash) => {
  try {
    console.log('🔐 === VERIFICANDO CONTRASEÑA CON FORMATO 6 PARTES ===');
    console.log('📝 Password ingresado:', inputPassword);
    console.log('🗄️ Hash almacenado completo:', storedHash);
    console.log('📏 Longitud hash:', storedHash?.length);
    ...
    if (storedHash.startsWith('$secure$')) {
      console.log('🔒 === MÉTODO 1: HASH SEGURO (6 PARTES) ===');
      
      const parts = storedHash.split('$');
      console.log('📊 Partes del hash:', parts.length, parts);
      ...
      if (parts.length === 6) {
        ...
        console.log('🔍 Componentes extraídos (6 partes):');
        ...
        console.log('🔍 === COMPARACIÓN EXACTA (6 PARTES) ===');
        console.log('Hash1 - Calculado:', finalHash1, '| Almacenado:', hash1Stored, '| ✅ Match:', finalHash1 === hash1Stored);
        console.log('Hash2 - Calculado:', finalHash2, '| Almacenado:', hash2Stored, '| ✅ Match:', finalHash2 === hash2Stored);
        console.log('Hash3 - Calculado:', finalHash3, '| Almacenado:', hash3Stored, '| ✅ Match:', finalHash3 === hash3Stored);
        console.log('Timestamp usado:', timestampStored);
        
        const match = finalHash1 === hash1Stored && 
                     finalHash2 === hash2Stored && 
                     finalHash3 === hash3Stored;
        
        console.log('🎯 ¿COINCIDEN TODOS LOS HASHES (6 PARTES)?', match);
        if (match) {
          console.log('✅ === PASSWORD CORRECTA CON FORMATO 6 PARTES ===');
          return true;
        }
      } 
      // ✨ FALLBACK PARA FORMATO DE 5 PARTES (por si acaso)
      else if (parts.length === 5) {
        console.log('🔄 Intentando formato de 5 partes como fallback...');
        ...
        console.log('🔍 Componentes extraídos (5 partes):');
        console.log('- Hash1:', hash1Stored);
        console.log('- Hash2:', hash2Stored);
        console.log('- Hash3:', hash3Stored);
        console.log('- Timestamp:', timestampStored);
        ...
        console.log('🔍 === COMPARACIÓN EXACTA (5 PARTES) ===');
        console.log('Hash1 - Calculado:', finalHash1, '| Almacenado:', hash1Stored, '| Match:', finalHash1 === hash1Stored);
        console.log('Hash2 - Calculado:', finalHash2, '| Almacenado:', hash2Stored, '| Match:', finalHash2 === hash2Stored);
        console.log('Hash3 - Calculado:', finalHash3, '| Almacenado:', hash3Stored, '| Match:', finalHash3 === hash3Stored);
        ...
        console.log('🎯 ¿COINCIDEN TODOS LOS HASHES (5 PARTES)?', match);
        if (match) {
          console.log('✅ === PASSWORD CORRECTA CON FORMATO 5 PARTES ===');
          return true;
        }
      } else {
        console.log('❌ Formato de hash inválido - partes:', parts.length, 'Se esperaban 5 o 6');
      }
    }
    
    // ✨ MÉTODO 2: COMPARACIÓN DIRECTA
    console.log('🔓 === MÉTODO 2: COMPARACIÓN DIRECTA ===');
    const directMatch = inputPassword === storedHash;
    console.log('🎯 ¿Coincide directamente?', directMatch);
    if (directMatch) {
    ...
    console.log('🔄 === MÉTODO 3: HASH LEGACY ===');
    try {
      ...
      console.log('🔍 Legacy hash calculado:', legacyHashString);
      console.log('🎯 ¿Coincide legacy?', legacyHashString === storedHash);
      
      if (legacyHashString === storedHash) {
        console.log('✅ === PASSWORD CORRECTA LEGACY ===');
        return true;
      }
    } catch (legacyError) {
...
export const login = async (email, password) => {
  try {
    console.log('🔍 === INICIANDO LOGIN ARREGLADO ===');
    console.log('📧 Email:', email);
    console.log('🔐 Password preview:', password ? `${password.substring(0, 2)}***` : 'N/A');
    ...
    console.log('Email:', user.var_email);
    console.log('Username:', user.var_username);
    console.log('Name:', user.var_name);
    console.log('Code:', user.int_user_code);
    console.log('Hash formato:', user.var_password?.substring(0, 15) + '...');
    console.log('Hash completo para debug:', user.var_password);
